...
waitforcounterchange
	...
	iret
;
;	Simple line render routine
;
renderline
; Fill in the line buffer we're going to output over spi	
	ldw x,#0
	ld a,#$00
	ld (y),a
	incw y	;Skip over first byte (We want $00 at either end
	ld a,linenumber
	cp a,#0
	jreq topOrBottom 
	cp a,#ScrLines
	...
	jrne blankLine
topOrBottom 
	ld a,#$FF	;Solid lines at top and bottom
	jra filllinebuff
blankLine		;Couple of blank lines at bottom of screen
	ld a,#$00
	jra filllinebuff
notBottom:
	ld a,#$81	  ; Draw some lines
filllinebuff
	ld (y),a
	incw y
	incw x
